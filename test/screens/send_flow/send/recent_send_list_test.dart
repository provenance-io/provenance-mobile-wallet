...
main() {
  group("RecentSendCell", () {
    ...
    testWidgets("Null Address", (tester)  async {
      ...
      expect(find.descendant(of: textFind, matching: find.text("View All")), findsOneWidget);
    });

    testWidgets("Not Null Address", (tester)  async {
      final recentAddress = RecentAddress("Address", DateTime.fromMicrosecondsSinceEpoch(0));
      ...
      expect(find.descendant(of: textFind, matching: find.text("Address")), findsOneWidget);
      expect(find.descendant(of: textFind, matching: find.text("12/31/69")), findsOneWidget);
    });
  });
  ...
  group("RecentSendList", () {
    ...
    void OnAddressClicked(RecentAddress address) => recentlyClickedAddress = address;

    final send1 = RecentAddress("Address1", DateTime.fromMicrosecondsSinceEpoch(0));
    final send2 = RecentAddress("Address2", DateTime.fromMicrosecondsSinceEpoch(0));

    Future<void> _build(WidgetTester tester, List<RecentAddress> recentAddresses) {
      return tester.pumpWidget(
        MaterialApp(
          home: RecentSendList(
            recentAddresses,
    ...
    testWidgets("Non-Empty List", (tester) async {
      ...
      expect(tester.widget<RecentSendCell>(cellFind.at(0)).recentAddress, send1);
      expect(tester.widget<RecentSendCell>(cellFind.at(1)).recentAddress, send2);
